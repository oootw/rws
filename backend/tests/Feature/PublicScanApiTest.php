...
it('возвращает tenant_not_found без заголовка X-Tenant-Slug', function (): void {
    $place = Place::factory()->create();

    $this->getJson(publicPlaceUrl($place))
        ->assertNotFound()
        ->assertJsonPath('code', 'tenant_not_found')
        ->assertJsonPath('message', 'Аккаунт не найден. Проверьте адрес сайта.');
});

it('возвращает tenant_not_found для неизвестного поддомена', function (): void {
    $place = Place::factory()->create();

    $this->getJson(publicPlaceUrl($place), [
        'X-Tenant-Slug' => 'missing-tenant',
...
it('возвращает place_not_found для выключенной точки', function (): void {
    $user = User::factory()->create(['subdomain_slug' => 'inactive-place']);
    $place = Place::factory()->for($user)->create(['is_active' => false]);
...
it('отклоняет redirect с недоступной площадкой', function (): void {
    $user = User::factory()->create(['subdomain_slug' => 'bad-platform']);
    $place = Place::factory()->for($user)->create();
    ...
    $this->postJson(publicPlaceUrl($place, '/redirect'), [
        ...
        ->assertUnprocessable()
        ->assertJsonValidationErrors(['platform_type'])
        ->assertJsonPath('errors.platform_type.0', 'Выбранная площадка недоступна.');
});
