...
it('retrieveById возвращает AdminUser для канонического id', function (): void {
    $user = makeProvider()->retrieveById(AdminUser::ID);
...
it('retrieveById возвращает null если admin не настроен', function (): void {
    expect(makeProvider(email: null)->retrieveById(AdminUser::ID))->toBeNull()
        ->and(makeProvider(hash: '')->retrieveById(AdminUser::ID))->toBeNull();
});
...
it('validateCredentials отклоняет пустой и не-строковый password', function (): void {
    $provider = makeProvider();
    $user = $provider->retrieveById(AdminUser::ID);
...
it('retrieveByToken всегда возвращает null (remember-me отключён)', function (): void {
    expect(makeProvider()->retrieveByToken(AdminUser::ID, 'token'))->toBeNull();
});
